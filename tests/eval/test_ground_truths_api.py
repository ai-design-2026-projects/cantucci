"""Smoke tests for the eval ground-truths endpoint.

Covers:
- GET /eval/ground-truths — 401 (no auth), 403 (non-admin), 200 empty list (admin).
"""
import pytest
...
class TestListGroundTruths:
    ...
    def test_non_admin_returns_403(self, client: TestClient, auth_header_user: dict) -> None:
        ...
        response = client.get("/eval/ground-truths", headers=auth_header_user)
    ...
    def test_admin_returns_empty_list(self, client: TestClient, auth_header_admin: dict) -> None:
        ...
        response = client.get("/eval/ground-truths", headers=auth_header_admin)
