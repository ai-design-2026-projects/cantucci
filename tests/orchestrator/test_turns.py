...
def patched_sessions(monkeypatch: pytest.MonkeyPatch):
    ...
    append = MagicMock()
    write = MagicMock()
    monkeypatch.setattr("backend.orchestrator.turn.terminal_paths.api_sessions.append_turn", append)
    monkeypatch.setattr("backend.orchestrator.turn.terminal_paths.api_sessions.write_feedback", write)
    return append, write
