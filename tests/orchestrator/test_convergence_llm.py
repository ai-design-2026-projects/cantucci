...
from backend.orchestrator.tools.convergence import check_llm_convergence
from backend.orchestrator.types import ConvergenceAction, ConvergenceCheckResponse
from backend.llm.types import LLMResponse


def _full(turns: list | None = None) -> MagicMock:
...
def _call(**overrides: Any):
    ...
    kwargs: dict[str, Any] = dict(
        ...
        full=_full(),
        preference_profile={"oracle_constraints": [], "final_cluster_name": "unknown", "final_cluster_description": None},
        cfg=_cfg(),
    )
...
class TestProceedDecision:
    ...
    def test_proceed_action(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "backend.orchestrator.tools.convergence.llm_harness.call",
...
class TestDryRunFixture:
    """The dry-run fixture for orchestrator_convergence validates against the schema."""
