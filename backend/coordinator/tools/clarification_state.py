...
def mark_awaiting(
    ...
    Optionally stores a structured ``PartitionSpec`` and ``target_cluster_id`` so that
    on the next turn the coordinator can use the proposed bins and original target cluster
    directly rather than relying on the intent agent to re-extract them from a short
    confirmation reply such as "Ok".
...
def take_awaiting(conversation_id: uuid.UUID) -> tuple[bool, PartitionSpec | None, uuid.UUID | None]:
    """
