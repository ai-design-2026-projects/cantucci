...
embedded catalog, the components that make up the conversational loop and the lifecycle of a single
turn.
...
\paragraph{Scrape:} Ingestion begins by downloading TMDB's daily catalog export, after which the surviving titles are enriched through detailed API requests with credits, user reviews, keywords, and trailer links.

\paragraph{Clean \& Filter:} To maintain data quality and processing efficiency, the pipeline discards adult content and applies a popularity threshold before those detailed requests, retaining only the titles worth embedding.
...
\paragraph{Fuse:}
Text and review embeddings are merged directly via a weighted average of their vectors since they share an underlying space.
...
After the Intent Agent converts the oracle message into an ordered action list, the coordinator routes that plan through the turn pipeline, deciding what to clarify, what to execute, and what state to persist.
...
\paragraph{Intent Agent:} The \textbf{strong-model} parser that maps the oracle's free-form message onto an \textbf{ordered list of actions}, each carrying a target cluster, operation parameters, and a confidence score.
A single message can resolve into several actions executed in order, so that later steps act on the result of earlier ones (for example, \emph{reset, then split by genre}).
...
\begin{itemize}[leftmargin=1.4em]
  \item \textbf{CLUSTER}: split a cluster (or the whole catalog) into subgroups, by free density, a concept axis, or a metadata partition;
...
\paragraph{Concept Agent:} Triggered exclusively when a request is \textbf{semantic} (concept-guided) rather than a deterministic partition, this \textbf{strong-model} agent takes as input only the concept string and a declared \textbf{space} (semantic or visual).
Because it is completely blind to the broader conversation, its output is a pure function of the concept itself: a \textbf{linear axis} represented as a unit vector.
To build it, the agent generates $k$ descriptive sentences for each of an engineered positive and negative pole, embeds them, and averages each set into a pole centroid; the axis then runs from the negative to the positive centroid.
Each film is then scored by the dot product of its embedding with this axis, within the chosen space.
...
When a cluster arises from a \textbf{deterministic partition} by a metadata attribute, the label is already implied and the agent assigns it deterministically.
...
In both scenarios, the agent evaluates each cluster's exemplar films alongside its inherited parent label, and the cluster's description is always produced by an LLM call, even when the label itself is deterministic. 
...
It sees the low-confidence action, the message behind it, and the current cluster labels.
...
\paragraph{Initialization and Intent Parsing:} The turn begins with the coordinator loading the active snapshot and labeling any remaining unlabeled clusters.
...
The system exposes two programmable interfaces and one human-facing interface, all thin surfaces over the coordinator and the data-access layer:
\begin{itemize}[leftmargin=1.4em]
  ...
  \item \textbf{Web frontend}: surfaces the entire human-facing experience, from starting and replaying conversations to inspecting clusters and their member films, visualizing and navigating the snapshot graph, and, for administrators, an evaluation lab that reviews run metrics, judge scores, and full transcripts;
