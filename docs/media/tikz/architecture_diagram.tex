    ...
    \draw[arrow] (decision.north) |-
        ...
        node[left, pos=0.3, font=\footnotesize] {SHOW}
        (orchestrator.south);

    % ---- Resolver Agent <-> Decision Agent (horizontal, same level) ----
    \draw[arrow] (decision.west) |-
        node[above, pos=0.75, font=\footnotesize] {ASK}
        (resolver.east);

    % ---- Resolver Agent to Orchestrator (feedback loop) ----
    \draw[arrow] (resolver.north) |-
        node[left, pos=0.3, font=\footnotesize] {Question}
        ([yshift=-0.25cm]orchestrator.west -| resolver.north) --
        ([yshift=-0.25cm]orchestrator.west);
