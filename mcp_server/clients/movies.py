...
class MoviesClient(BaseClient):
    ...
    async def get_movie(self, movie_id: int) -> dict:
        """GET /movies/{id} — full metadata for one movie by TMDB ID.
        ...
        """
        resp = await self._http.get(f"/movies/{movie_id}")
        self._check(resp)
        return resp.json()

    async def get_movies_batch(self, movie_ids: list[int]) -> list:
        """POST /movies/batch — full metadata for up to 200 movies in one call.
        ...
        """
        resp = await self._http.post("/movies/batch", json={"ids": movie_ids})
        self._check(resp)
        return resp.json()
