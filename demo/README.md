...
├── recordings/              Recorded sessions (JSON — gitignored)
├── utils/
...
└── playwright/
    ...
    └── lib/
        ├── constants.ts     Shared timing and selector constants
...
All knobs are in [`configs/demo.yaml`](../configs/demo.yaml).

Backend timing (replay step delays) is read from the `backend:` section at startup. Playwright URLs are overridable via environment variables; `configs/demo.yaml` documents their defaults.

| Variable | Description |
|---|---|
| `CINEPAL_DEMO_MODE` | `record` or `replay`; unset for live mode |
| `CINEPAL_DEMO_RECORDING` | Path to the recording JSON file (backend) |
